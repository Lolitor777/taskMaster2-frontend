  ...
  return (
    <div className="list-container">
      <div className="filter-container">
        <button className="btn btn-filter-1" onClick={getTask}>
          Mostrar todas
        </button>
        <button className="btn btn-filter-2" onClick={pendingTask}>
          Tareas pendientes
        </button>
        <button className="btn btn-filter-1" onClick={taskInProgress}>
          Tareas en progreso
        </button>
        <button className="btn btn-filter-2" onClick={completeTask}>
          Tareas completadas
        </button>
      </div>
      {
                ...
                {task.map((element) => (
                  <tr key={element.id} className="content-table">
                    ...
                    <td className="text-center">
                      {element.tb_priority.description}
                    </td>
                    <td className="text-center">
                      {element.tb_state.description}
                    </td>
                    <td className="text-center">
                      <Link to={`/edit/${element.id}`} className="btn btn-edit">
                        <box-icon
                          name="edit-alt"
                          type="solid"
                          color="#fff"
      ...
      <Link to={"/crear"} className="btn btn-create">
        Crear
      </Link>
    </div>
  );
